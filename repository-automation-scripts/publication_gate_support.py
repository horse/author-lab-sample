...
def resolve_persona_model(
    ...
    for directory in project.get("derived_author_persona_directories", []):
        ...
        if persona_manifest.get("derived_author_id") != persona_id:
            continue
        ...
        matches.append((persona_root, persona_manifest, model_manifest))
    if len(matches) != 1:
...
def validate_approved_work_item(
    ...
    _, _, model_manifest = resolve_persona_model(
        repository_root, project, state["derived_author_id"]
    )
...
def validate_publication_record(
    ...
    *,
    ...
    if missing:
        raise ValueError(
            f"Publication {record.get('publication_id', '<unknown>')} missing fields: "
            f"{missing}"
        )
    ...
    if record["publication_status"] in {"approved", "published"}:
        final_file = validate_approved_work_item(
            repository_root, project, state, work_root
        )
    else:
        final_file = work_root / "final-approved-article.md"
    ...
    if require_canonical_file and record["publication_status"] in {
        ...
        if sha256_file(canonical_path) != sha256_file(final_file):
            raise ValueError(
                f"Publication {record['publication_id']} canonical file differs from "
                "the approved work-item final"
            )
...
def collect_publication_records(
    ...
    for metadata_path in publication_root.glob("**/publication-metadata.json"):
        if ".publication-staging" in metadata_path.parts:
            continue
