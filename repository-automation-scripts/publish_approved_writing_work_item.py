...
def validate_publication_identifier(value: str) -> str:
    if not re.fullmatch(r"publication-\d{4}-\d{3}-[a-z0-9]+(?:-[a-z0-9]+)*", value):
...
def publish_approved_work_item(
    ...
    if publication_category not in ALLOWED_PUBLICATION_CATEGORIES:
        raise ValueError(
            f"Unknown publication category {publication_category!r}; allowed categories are {sorted(ALLOWED_PUBLICATION_CATEGORIES)}"
        )
    if publication_status not in {"approved", "published"}:
        raise ValueError("Publication transaction accepts only approved or published status")
    ...
    final_file = validate_approved_work_item(repository_root, project, state, work_root)
    ...
    validate_publication_record(repository_root, project, metadata)
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Publish one approved writing work item through an atomic publication gate."
    )
